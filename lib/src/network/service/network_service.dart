...
import 'package:qunzo_agent/src/network/response/api_response.dart';
...
class NetworkService extends getx.GetxService {
  ...
  Future<ApiResponse<Map<String, dynamic>>> register({
    ...
    if (data is FormData) {
      _log('📦 Register Body (FormData Fields): ${data.fields}');
      _log('📦 Register Body (FormData Files): ${data.files}');
    }

    try {
      final response = await _dio.post(
        ApiPath.registerEndpoint,
        data: data is Map<String, dynamic> ? jsonEncode(data) : data,
      );
  ...
  ApiResponse<Map<String, dynamic>> _handleDioException(
    ...
    ToastHelper().showErrorToast('An error occurred. Please try again.');
